...
public class AdapterDelegatesManager<T> {
  ...
  protected Set<Integer> ignoreTypes = new HashSet<>();
  /**
  ...
  public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    if (delegate == null) {
//      if (viewType==10000 || viewType==10001)  return null;
      if (ignoreTypes.contains(viewType)) {
        return null;
      }
  ...
  public void onBindViewHolder(@NonNull T items, int position,
    ...
    if (delegate == null) {
//      if (viewHolder.getItemViewType()==10000 || viewHolder.getItemViewType()==10001)  return;
      if (ignoreTypes.contains(viewHolder.getItemViewType())) {
        return;
      }
...
//      if (viewHolder.getItemViewType()==10000 || viewHolder.getItemViewType()==10001)  return;
  ...
  public boolean onFailedToRecycleView(@NonNull RecyclerView.ViewHolder viewHolder) {
    AdapterDelegate<T> delegate = getDelegateForViewType(viewHolder.getItemViewType());
    if (delegate == null) {
//      if (viewHolder.getItemViewType()==10000 || viewHolder.getItemViewType()==10001)  return false;
      if (ignoreTypes.contains(viewHolder.getItemViewType())) {
        return false;
      }
      throw new NullPointerException("No delegate found for "
...
//      if (viewHolder.getItemViewType()==10000 || viewHolder.getItemViewType()==10001)  return;
  ...
  public void addIgnoreTypes(List<Integer> ignoreTypes){
    if (ignoreTypes != null) {
      this.ignoreTypes.addAll(ignoreTypes);
    }
  }

  public void addIgnoreType(int ignoreType) {
    ignoreTypes.add(ignoreType);
  }
