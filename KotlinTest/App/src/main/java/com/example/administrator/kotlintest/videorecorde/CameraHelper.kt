...
//            val bestPreviewSize = getBestSize(mSurfaceView.width, mSurfaceView.height, mParameters.supportedPreviewSizes)
            val bestPreviewSize = getBestSize(picWidth, picHeight, mParameters.supportedPreviewSizes)
            bestPreviewSize?.let {
    ...
    private fun startFaceDetect() {
        mCamera?.let {
            ...
            it.setFaceDetectionListener { faces, _ ->
                ...
                Log.e("camerHelp","检测到 ${faces.size} 张人脸")
            }
        }
    }
    ...
    private fun isSupportFocus(focusMode: String): Boolean {
        ...
        for (mode in listFocusMode) {
            if (mode == focusMode)
                autoFocus = true
            Log.e("camerHelp","相机支持的对焦模式： $mode")
        }
        return autoFocus
    }
    ...
    private fun getBestSize(targetWidth: Int, targetHeight: Int, sizeList: List<Camera.Size>): Camera.Size? {
        ...
        for (size in sizeList) {
            ...
            Log.e("camerHelp","系统支持的尺寸 : ${size.width} * ${size.height} ,    比例$supportedRatio")
        }
        ...
        Log.e("camerHelp","目标尺寸 ：$targetWidth * $targetHeight ，   比例  $targetRatio")
        Log.e("camerHelp","最优尺寸 ：${bestSize?.height} * ${bestSize?.width}")
        return bestSize
    }
    ...
    private fun setCameraDisplayOrientation(activity: Activity) {
        ...
        Log.e("camerHelp","屏幕的旋转角度 : $rotation")
        Log.e("camerHelp","setDisplayOrientation(result) : $mDisplayOrientation")
    }
